...
function formulaes_fetch_and_install {
    ...
    brew_fetch_list "${list_of_formulaes[@]}" > /dev/null 2>&1
    brew_install_list "${list_of_formulaes[@]}" > /dev/null 2>&1
...
function casks_fetch_and_install {
    ...
    brew_cask_fetch_list "${list_of_casks[@]}" > /dev/null 2>&1
    brew_cask_install_list "${list_of_casks[@]}" > /dev/null 2>&1
    printf "%s " "${list_of_casks[@]}" | tr "\n" " " | sort
}

function casks_sudo_fetch_and_install {
    raw_list_of_cask=$(find ./casks/ -type f -name 'sudo_needed.txt' | xargs cat)
    list_of_casks=($raw_list_of_cask)
    brew_cask_fetch_list "${list_of_casks[@]}" > /dev/null 2>&1
    brew_cask_install_list "${list_of_casks[@]}" > /dev/null 2>&1
    printf "%s " "${list_of_casks[@]}" | tr "\n" " " | sort
